...
def _default_compute_score_sm(data_source, solution_str, ground_truth, extra_info=None):
    if data_source == 'hotpotqa/hotpot_qa' or data_source == 'bdsaglam/musique' or data_source == 'xanhho/2WikiMultihopQA':
        ...
        from . import gsm8k
        res = gsm8k.compute_score_sm(solution_str, ground_truth)
    elif data_source == 'BytedTsinghua-SIA/DAPO-Math-17k':
        from . import retool
        res = retool.compute_score_sm(solution_str, ground_truth)
    else:
        raise NotImplementedError
...
def _default_compute_score_sim(data_source, solution_str, ground_truth, extra_info=None):
    if data_source == 'hotpotqa/hotpot_qa' or data_source == 'bdsaglam/musique' or data_source == 'xanhho/2WikiMultihopQA':
        from . import qa_em_and_format
        res = qa_em_and_format.compute_semantic_similarity(solution_str, ground_truth)
    elif data_source == 'openai/gsm8k':
        from . import gsm8k
        res = qa_em_and_format.compute_semantic_similarity(solution_str, ground_truth)
    elif data_source == 'BytedTsinghua-SIA/DAPO-Math-17k':
...
def _default_compute_score(data_source, solution_str, ground_truth, extra_info=None):
    return {
        ...
        # "sim": _default_compute_score_sim(data_source, solution_str, extra_info),
    }