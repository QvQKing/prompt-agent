...
import json  # JSON编解码
import openai
API_KEY = "sk-v46hWyxfMF72b2T4603d6c0638Cf45078c027d1cE3E8C33a"  # 替换为你的 OpenAI API Key
...
class SearchTool(BaseTool):
    ...
    description = "Provide answers based on the given question and question analysis"  
    # 工具功能描述
    parameters = {
        "type": "object",  # 输入参数类型是对象
        "properties": {    # 对象的属性
            "prompt": {"type": "string", "description": "Provide ansewer to question"}  # 必须有一个query字段（字符串）
        },
    ...
    def batch_execute(self, args_list: List[Dict]) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # 提取所有查询字符串; 需要修改1.在这里改GPT的提示，给GPT新的提示
            queries = [x["prompt"] for x in args_list]
            ...
            # 返回成功标志的结果列表；这里是GPT返回的，参考当前代码的格式进行返回
